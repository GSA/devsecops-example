resource "aws_cloudwatch_metric_alarm" "high_memory_wordpress" {
  ...
  period                    = "300"
  statistic                 = "Average"
  threshold                 = "${var.high_memory_wordpress_alarm_threshold}"
...
resource "aws_cloudwatch_metric_alarm" "high_cpu_wordpress" {
  ...
  alarm_description         = "This metric monitors ec2 cpu utilization and alarms for 80% over two eval periods on instance ${aws_instance.wordpress.id}"
  alarm_actions = ["${aws_sns_topic.sns_general_availability.arn}"]
